...
class CosyVoice2(CosyVoice):
    ...
    def inference_zero_shot_w_mid_state(self, tts_text, prompt_text_A, prompt_text_B, 
                                prompt_speech_16k_A, prompt_speech_16k_B,
                                zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True):
        """
        Dual-emotion with intermediate state zero-shot inference with automatic emotion transfer.
        
        Args:
            tts_text: Text to synthesize
            prompt_text_A: Prompt text for emotion A
            prompt_text_B: Prompt text for emotion B
            prompt_speech_16k_A: Prompt audio for emotion A (16kHz)
            prompt_speech_16k_B: Prompt audio for emotion B (16kHz)
            zero_shot_spk_id: Speaker ID (optional)
            stream: Streaming mode (currently False only)
            speed: Speech speed multiplier
            text_frontend: Whether to apply text normalization
        
        Yields:
            Dict with 'tts_speech' key containing audio tensor
        """
        # Normalize texts
        prompt_text_A = self.frontend.text_normalize(prompt_text_A, split=False, text_frontend=text_frontend)
        prompt_text_B = self.frontend.text_normalize(prompt_text_B, split=False, text_frontend=text_frontend)
        
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            # Process emotion A prompt
            model_input_A = self.frontend.frontend_zero_shot(
                i, prompt_text_A, prompt_speech_16k_A, self.sample_rate, zero_shot_spk_id
            )
            
            # Process emotion B prompt
            model_input_B = self.frontend.frontend_zero_shot(
                i, prompt_text_B, prompt_speech_16k_B, self.sample_rate, zero_shot_spk_id
            )
            
            # Merge inputs for dual-emotion mode with intermediate state
            model_input = model_input_A.copy()
            model_input['flow_embedding_B'] = model_input_B['flow_embedding']
            model_input['prompt_speech_feat_B'] = model_input_B['prompt_speech_feat']
            model_input['flow_prompt_speech_token_B'] = model_input_B['flow_prompt_speech_token']
