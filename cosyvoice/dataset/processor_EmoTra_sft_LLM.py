...
# Modified by Tianchi Liu, March 2026 — EmoTra SFT_LLM data processor
#
...
def parse_vad_hidden_states_sft_LLM(data, mode='train', **kwargs):
    """
    Parse VAD features and hidden states for SFT_LLM.
    Converts raw vad_a/vad_b floats and hidden_states numpy arrays to torch tensors.
    """
    for sample in data:
        if 'vad_a' in sample:
...
def padding_EmoTra_sft_LLM(data, use_spk_embedding, mode='train', gan=False, dpo=False):
    """
    Padding function for EmoTra SFT_LLM.
    Collates a batch: text_token, speech_token, speech_feat, embeddings, VAD, hidden states.
    """
    for sample in data:
